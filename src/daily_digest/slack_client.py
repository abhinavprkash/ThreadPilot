...
class MockSlackClient:
    ...
    def get_all_users(self, exclude_bots: bool = True) -> list[dict]:
        """Get all users from fixtures."""
        users = self._data.get("users", {})
        user_list = []
        for user_id, user_data in users.items():
            if exclude_bots and user_data.get("is_bot", False):
                continue
            user_list.append({
                "id": user_id,
                "name": user_data.get("name", user_id),
                "real_name": user_data.get("real_name", user_data.get("name", user_id)),
        ...
        try:
            ...
            while True:
                ...
                for member in response.get("members", []):
                    # Skip bots and deleted users if requested
                    if exclude_bots and (member.get("is_bot", False) or member.get("deleted", False)):
                        continue
...
class SlackClient:
    ...
    def get_all_users(self, exclude_bots: bool = True) -> list[dict]:
        """Get all users from workspace."""
        return self._client.get_all_users(exclude_bots)
